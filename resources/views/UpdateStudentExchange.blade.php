...
<head>
    ...
    <title>Add Exchange Program Student</title>
    ...
    <div class="d-flex " style="height: 100%;">
        ...
        <div class="col-md-9 p-5" style="background-color:#EFEFEF">
            <div class="p-1 d-flex align-items-center justify-content-between " style="background-color:#EFEFEF;margin-top:-30px">
                <h3 class="text-dark mb-3">Add Exchange Program Student</h3>
                @include('dropdown_admin')
            </div>
            <h4 class="mb-5" style="color:#967117;" >Student</h4>
            <div class="p-5 w-100 mx-auto" style="background-color:white;">
                <h4 class="text-dark mb-5">Exchange Program</h4>
                <form method="post" action="/updateStudentExtchange/{{$dataExchS->id}}">
                    @csrf
                    <div class="row">
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              <label for="lastname" class="form-label">Description</label>
                              <input type="text" class="form-control" value="{{$dataExchS->description}}" name="description" id="lastname">
                            </div>
                        </div>
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              ...
                              <select id="" name="type" class="form-select">
                                <option value=""></option>
                                <option value="Internship" {{ $dataExchS->type == 'Internship' ? 'selected' : '' }}>Internship</option>
                              </select>
                            </div>
                        </div>
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              ...
                              <input type="date" value="{{$dataExchS->date_start}}" name="date_start" class="form-control" id="date">
                            </div>
                        </div>
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              <label for="date" class="form-label">To</label>
                              <input type="date" name="date_end" value="{{$dataExchS->date_end}}" class="form-control" id="date">
                            </div>
                        </div>
                        <div class="form-group m-5 ">
                            <label for="student_id" class="mb-4">Students engaged in the excahnge :</label></br>
                            @foreach($students as $student)
                                <input class="mx-5 form-check-input" name="students[]" type="checkbox" value="{{$student->id}}" id="{{$student->firstname}} {{$student->lastname}}" {{ in_array($student->id, $dataExchS->students->pluck('id')->toArray()) ? 'checked' : '' }}>

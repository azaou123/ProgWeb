    ...
    <div class="d-flex " style="height: 100%;">
        ...
        <div class="p-1 d-flex align-items-center justify-content-between " style="background-color:#EFEFEF;margin-top:-30px">
            ...
            <div class="p-5 w-100 mx-auto" style="background-color:white;">
                ...
                <form  method="post" enctype="multipart/form-data" action="/updateInternship/{{$dataI->id}}">
                    @csrf
                    <div class="row">
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              <label for="title" class="form-label" style="color:grey">Title</label>
                              <input type="title" name="title" value="{{$dataI->title}}" class="form-control" id="title">
                            </div>
                        </div>
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              <label for="" class="form-label" style="color:grey">Description</label>
                              <input type="description" value="{{$dataI->description}}" name="description" class="form-control" id="description">
                            </div>
                        </div>
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              <label for="" class="form-label" style="color:grey">From</label>
                              <input type="date" name="from" value="{{$dataI->date_start}}" class="form-control" id="">
                            </div>
                        </div>
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                            <label for="" class="form-label" style="color:grey">To</label>
                            <input type="date" name="to" value="{{$dataI->date_end}}" class="form-control" id="">
                             </div>
                        </div>
                        <div class="col-6">
                            <div class="mb-3">
                              <label for="" class="form-label" style="color:grey">Company</label>
                              <input type="text" name="company" value="{{$dataI->company}}" class="form-control" id="">
                            </div>
                        </div>
                        <div class="form-group m-5">
                            <label for="teacher_id" class="mb-4">Supervisors:</label><br>
                            @foreach($teachers as $teacher)
                                <input class="mx-5 form-check-input" name="teachers[]" type="checkbox" value="{{ $teacher->id }}" id="{{ $teacher->firstname }}_{{ $teacher->lastname }}" {{ in_array($teacher->id, $dataI->teachers->pluck('id')->toArray()) ? 'checked' : '' }}>
                                <label class="mx-1 form-check-label" for="{{ $teacher->firstname }}_{{ $teacher->lastname }}">{{ $teacher->firstname }} {{ $teacher->lastname }}</label><br>
                            @endforeach
                        </div>

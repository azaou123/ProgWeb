    ...
    <div class="d-flex" style="height: 100%;">
        ...
        <div class="col-md-9 p-3 px-5 pt-5" style="background-color: #efefef">
            ...
            <div class="p-4 border rounded shadow bg-white">
                ...
                <form method="post" action="/updateTeacherExchange/{{$dataExchT->id}}">
                    @csrf
                    <div class="row">
                        <div class="col-12 mb-4">
                            <div class="mb-3">
                              <label for="lastname" class="form-label">Description</label>
                              <textarea class="form-control" rows="5" name="description" id="comment" placeholder="Your description...">{{$dataExchT->description}}</textarea>
                            </div>
                        </div>
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              <label for="date" class="form-label">From</label>
                              <input type="date" value="{{$dataExchT->date_start}}"  name="date_start" class="form-control" id="date">
                            </div>
                        </div>
                        <div class="col-6 mb-4">
                            <div class="mb-3">
                              <label for="date" class="form-label">To</label>
                              <input type="date" name="date_end" value="{{$dataExchT->date_end}}" class="form-control" id="date">
                            </div>
                        </div>
                    </div>
                    <div class="form-group m-5">
                            <label for="teacher_id" class="mb-4">teachers engaged in the exchange:</label><br>
                            @foreach($teachers as $teacher)
                                <input class="mx-5 form-check-input" name="teachers[]" type="checkbox" value="{{ $teacher->id }}" id="{{ $teacher->firstname }} {{ $teacher->lastname }}" {{ in_array($teacher->id, $dataExchT->teachers->pluck('id')->toArray()) ? 'checked' : '' }}>
